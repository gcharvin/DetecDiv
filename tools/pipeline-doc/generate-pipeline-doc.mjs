...
function visualBlock(node, examples) {
  ...
  return `<div class="example-grid example-count-${Math.min(items.length,2)}">${items.slice(0,2).map(item => `<figure><div class="visual-frame">${item.kind ? `<span class="visual-kind visual-${esc(item.kind)}">${esc(item.kind)}</span>` : ''}<img src="${item.src}" alt="${esc(item.alt || item.caption || 'Project example')}"></div><figcaption>${esc(item.caption || 'Example from the associated project')}</figcaption></figure>`).join('')}</div>`;
}
