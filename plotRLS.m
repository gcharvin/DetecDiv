function hrls=plotRLS(handle,rls,param)
...
if nargin<3 % no parameter provided build a param variable 
    param=[];
...
cmap2(:,3)=(fliplr(l))';
    ...
    param.sort=0; % 1 if sorting of trajectories according to size
...
Traj(rec2,'Color',param.colormap,'colorindex',cindex2,'width',param.cellwidth,'startX',startX,'startY',startY,'sepwidth',param.sepwidth,'sepColor',param.sepcolor,'edgeWidth',1,'gradientwidth',param.gradientWidth,'tag',['Trap - ' num2str(rls(ix(i)).trap)]);
...
if numel(rls)>1
text(30,35, ['Median RLS= ' num2str(med)],'FontSize',20);
end
