...
for i=fovid
    ...
    for l=1:numel(tmpfov(i).roi)
        ...
        rroitmp(3)=min(rroi(1)+rroi(3)-1,size(list{1,1},2));
        rroitmp(4)=min(rroi(2)+rroi(4)-1,size(list{1,1},1));
        
        tmpfov(i).roi(l).image=uint16(zeros(rroitmp(4)-rroitmp(2)+1,rroitmp(3)-rroitmp(1)+1,numel(tmpfov(i).srclist),numel(nframes)));
        ...
        for j=1:numel(nframes)
            ...
            for k=1:numel(tmpfov(i).srclist)
                ...
                %bug: taille de tmpfov(i).roi(l).image(:,:,k,j) différente de
                %tmp(rroitmp(2):rroitmp(4),rroitmp(1):rroitmp(3)) quand roi
                %dépasse. rroi peut commencer à 0 ou finir à 1025, mais pas rroitmp
                %du cadre. exemple fov(10).roi(48)
            end
            %cc=cc+1;
        end
