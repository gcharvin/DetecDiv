...
%     name=1;
% else
%     prompt='Data type: list of Images-->0;  PhyloCell project-->1;  folder with 4D Tiff files-->2; MicroManager folder -->3; (Default: 0)';
%     name= input(prompt);
%     if numel(name)==0
%         name=0;
%     end
...
%         list=dir(pathe);
%         
%         files=list(contains({list.name},{'.tif'}));
%         pathe=files.folder;
%         realfiles=cellfun(@(x) fullfile(pathe , x),{files.name},'UniformOutput',false);
%         
%         %REORDER POS
%         
%         realfolders=realfiles;
%         
%         cc=1;
%                 posnum=cellfun(@(x) extractBetween(x,'_s','.'), realfolders,'UniformOutput',false);
%                posnum=cellfun(@(x) str2num(x{:}),posnum);
%                 [~,Sidx] = sort(posnum);
%                 realfolderstmp=realfolders(Sidx);
% 
%         realfiles=realfolderstmp;
%     
%           if size(realfiles,1)==0
%             disp('Error : there is no file within the selected folder !')
%             return;
%           end
%           
%          disp(realfiles')
%         prompt=['Please enter the positions to import (using Matlab syntax); Default: 1:' num2str(size(realfiles',1)) ' ']
%         
%            npos= input(prompt,'s');
%         if numel(npos)==0
...
%         %                 list=list(occ);
% 
%         disp(['OK, we will import files IDs: ' num2str(npos)]);
%         
%         n=numel(obj.fov);
% 
%         if n==1
