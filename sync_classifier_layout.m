...
code = regexprep(code, ...
    '(?m)^\s*app\.SetboundsselectionrulesButton\.Text\s*=\s*''[^'']*'';', ...
    '            app.SetboundsselectionrulesButton.Text = ''Set training frames...'';', 'once');
...
code = regexprep(code, ...
    '(?m)^\s*app\.AnnotationFilterDropDown\.Items\s*=\s*\{[^\r\n]+\};', ...
    ['            app.AnnotationFilterDropDown.Items = ' ...
     '{''All'', ''Missing'', ''Draft'', ''Invalid'', ''Ready''};'], 'once');
end
