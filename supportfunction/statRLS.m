...
plot(0:max(max(rlstNdivs),max(rlsgNdivs)),0:max(max(rlstNdivs),max(rlsgNdivs)),'k','LineStyle','--','LineWidth',2);
box on
...
%     set(gcf,'Units','centimeters','Position', [5 5 xf_width+3 yf_width+3]);
    ...
    set(ax,'FontSize',8,'LineWidth',1,'FontWeight','bold','XTick',[0:25:200],'TickLength',[0.02 0.02]); %put thiner axes and line
    ax.Children(1).LineWidth=1;
    ax.Children(2).LineWidth=1;
