...
% deployment because the API VM does not have direct project storage access.
    ...
    ip.addParameter('ScanOrphanRaw', false, @(x)islogical(x) || isnumeric(x));
    ip.addParameter('QueuePreviews', false, @(x)islogical(x) || isnumeric(x));
    ...
    payload = struct( ...
        ...
        'visibility', char(string(opts.Visibility)), ...
        'clear_existing_for_root', logical(opts.ClearExistingForRoot), ...
        'scan_orphan_raw', logical(opts.ScanOrphanRaw), ...
        'queue_previews', logical(opts.QueuePreviews), ...
