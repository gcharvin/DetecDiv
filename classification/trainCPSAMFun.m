...
%   /split  : [N 1]     (uint8)   -> 1 = train, 0 = val (interne DD / framebank)
%
% Un sous-ensemble de ces images (fraction CPSAM_ValFraction) est utilisé
% comme jeu de validation/test pour CellposeSAM.
%
% -------------------------------------------------------------------------
...
ValFraction = max(0, min(trainingParam.CPSAM_ValFraction, 0.5));  % clamp 0..0.5
...
% -------------------------------------------------------------------------
% 4) Script Python – avec Seed, validation set & best model
% -------------------------------------------------------------------------
...
"\n" + ...
"def load_from_framebank(framebank_path, val_fraction, seed=None):\n" + ...
"    # Charge images/masks depuis le framebank et split en train/val\n" + ...
...
"            )\n" + ...
"\n" + ...
"        # indices avec split == 1 : candidats train/val\n" + ...
"        train_idx = np.where(split == 1)[0]\n" + ...
"        print(f'[INFO] found {len(train_idx)} candidate train/val frames (split==1)')\n" + ...
"\n" + ...
"        if seed is not None:\n" + ...
...
"        if n_total == 0:\n" + ...
"            raise RuntimeError('No frames with split==1 found in framebank.')\n" + ...
"\n" + ...
...
"        n_val = max(0, n_val)\n" + ...
"\n" + ...
"        val_idx  = np.sort(train_idx[:n_val]) if n_val > 0 else np.array([], dtype=int)\n" + ...
"        train_idx = np.sort(train_idx[n_val:])\n" + ...
"        print(f'[INFO] using {len(train_idx)} frames for training and {len(val_idx)} for validation (val_fraction={val_fraction:.3f})')\n" + ...
"\n" + ...
"        imgs = []\n" + ...
"        labels = []\n" + ...
"        val_imgs = []\n" + ...
...
"\n" + ...
"    val_fraction   = %g\n" + ...
"    imgs, labels, val_imgs, val_labels = load_from_framebank(framebank_path, val_fraction, seed=seed)\n" + ...
