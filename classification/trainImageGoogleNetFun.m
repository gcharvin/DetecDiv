...
fprintf('-------------------------------\n');
    ...
    case 'hdf5'
        ...
        % On ne suppose plus uniquement "<strid>_framebank.h5",
        % mais on cherche tous les "<strid>_framebank*.h5" et on
        % prend le plus récent lisible.
        ...
        % Trier par date de modification décroissante (plus récent d'abord)
        ...
        for k = 1:numel(d)
            ...
            try
                % Petit test de lisibilité : si ça passe, on l'utilise
                h5info(cand);
                h5File = cand;
                break;
            catch ME
                warning('[WARN] Unreadable framebank candidate %s (%s). Skipping...', ...
                    cand, ME.message);
            end
        end
