...
if isfield(classif.trainingParam, 'cellprob_threshold') && ~isempty(classif.trainingParam.cellprob_threshold)
    cellprob_threshold = classif.trainingParam.cellprob_threshold;
else
...
py_script = sprintf( ...
    ...
    num2str(flow_threshold), ...
    num2str(cellprob_threshold), ...
    round(min_size), ...
    classif_path_clean ...
);
