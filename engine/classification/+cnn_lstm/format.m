...
% CNN_LSTM.format
% Format LSTM training set (HDF5/TIFF) for a given classifier.
%
% Sections:
%   1) Read trainingParam + ctx overrides
%   2) ROI selection + frame export
%   3) Framebank finalization + artifact summary

% Tous les paramètres de formatage (fraction, seed, crop, backend, …)
% sont lus dans classif.trainingParam (prefixe 'Format_').
% Les overrides optionnels passent par ctx.params.*
...
% -------------------------------------------------------------------------
Frames     = [];        % peut être surchargé par ctx
...
% -------------------------------------------------------------------------
% 2) ctx overrides (frames + Format_* options)
% -------------------------------------------------------------------------
if isfield(ctx, 'sel') && isfield(ctx.sel, 'frames') && ~isempty(ctx.sel.frames)
    Frames = ctx.sel.frames;
end
if isempty(Frames) && isfield(params, 'frames')
...
Fraction            = cnn_lstm.utils.getParam(params, {'Fraction','Format_Fraction'}, Fraction);
Seed                = cnn_lstm.utils.getParam(params, {'Seed','Format_Seed'}, Seed);
Crop                = cnn_lstm.utils.getParam(params, {'Crop','Format_Crop'}, Crop);
CropCenter          = cnn_lstm.utils.getParam(params, {'CropCenter','Format_CropCenter'}, CropCenter);
CropSize            = cnn_lstm.utils.getParam(params, {'CropSize','Format_CropSize'}, CropSize);
UndersampleMajority = cnn_lstm.utils.getParam(params, {'UndersampleMajority','Format_UndersampleMajority'}, UndersampleMajority);
StorageBackend      = cnn_lstm.utils.getParam(params, {'StorageBackend','Format_StorageBackend'}, StorageBackend);

% Normalize backend value if passed as popup cell
if iscell(StorageBackend)
    StorageBackend = StorageBackend{end};
end

% Dériver UseHDF5 à partir du backend (après overrides)
UseHDF5 = strcmpi(string(StorageBackend),'hdf5');
WriteTiffImages= ~UseHDF5; 

out = cnn_lstm.utils.outInitSafe('cnn_lstm.format');
outputCount = 0;
...
try
    channel = classif.getInputChannels();
catch
...
if isempty(rois_sel)
    ...
    out.ok = false;
    out.status = "SKIP";
    out.warnings{end+1} = 'No ROI selected for LSTM formatting.';
    return;
end
...
if UndersampleMajority < 1 && (UseHDF5 || WriteTiffImages) && strcmp(category,'LSTM')
    ...
    for k = 1:numel(rois_sel)
        ...
        fraLoc = trainingBounds.frames(classif,ridx,nLab,Frames, ...
            'RoiPosition',k);
        if isempty(fraLoc)
            continue;
        end
...
for i = 1:numel(rois_sel)
    ...
    bounds = trainingBounds.resolve(classif,ridx,'FrameCount',size(im,4));
    if isempty(bounds)
    ...
    fra = trainingBounds.frames(classif,ridx,size(im,4),Frames, ...
        'RoiPosition',i);
...
out.metrics.nRois = numel(rois_sel);
out.artifacts.trainingFolder = fullfile(classif.path, foldername);
out.artifacts.backend = StorageBackend;
if UseHDF5
    out.artifacts.framebank = h5Framebank;
