...
% CNN_LSTM.train
% Train CNN backbone + LSTM head (pipeline-friendly entry).
%
% Sections:
%   1) Mode handling (init/train)
%   2) Dataset checks + CNN backbone
%   3) Activation extraction + LSTM training
%   4) Assemble full network + artifacts
...
out = cnn_lstm.utils.outInitSafe('cnn_lstm.train');
...
if isfield(ctx, 'params') && isstruct(ctx.params) && ~isempty(ctx.params)
    classif.trainingParam = cnn_lstm.utils.applyParamOverrides(classif.trainingParam, ctx.params);
end
