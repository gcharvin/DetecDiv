...
elseif any(strcmp(backend,{'continuous_cell_state','scene_parent_v54'}))
    cfg.frame_interval_minutes = double(param.frameIntervalMinutes);
    cfg.checkpoint = struct( ...
    ...
    if strcmp(backend,'scene_parent_v54')
        cfg.scene_parent_runtime_manifest = normalizedPath( ...
            param.sceneParentRuntimeManifestPath);
        cfg.annotation_parent_reranker_manifest = '';
        if ~isempty(param.annotationParentRerankerManifestPath)
            cfg.annotation_parent_reranker_manifest = normalizedPath( ...
                param.annotationParentRerankerManifestPath);
        end
        cfg.annotation_budneck_model_manifest = '';
        cfg.annotation_budneck_marker_identity = '';
        if annotationBudneckEnabled(ctx,param,observations)
            cfg.annotation_budneck_model_manifest = normalizedPath( ...
                param.annotationBudneckModelManifestPath);
            cfg.annotation_budneck_marker_identity = char(string( ...
                param.budneckMarkerIdentity));
        end
        cfg.causal_solver_feedback = false;
    end
else
...
runtime = cellLatentModel.utils.runPythonModule( ...
    'infer-roi',configPath,runtimeCtx,stdoutPath);
...
if any(strcmp(backend,{'continuous_cell_state','scene_parent_v54'}))
    validateContinuousResult(result,cfg);
end
...
parameters = struct( ...
    ...
    'max_parent_centroid_distance', ...
        double(p.maxParentCentroidDistance), ...
    'max_parent_contour_distance', ...
        double(p.maxParentContourDistance), ...
    'tracking_load_guard_enabled',logical(p.trackingLoadGuard), ...
    'tracking_load_guard_max_new_tracks', ...
        double(p.maxNewTracksPerFrame));
end

function validateContinuousResult(result,cfg)
if ~isstruct(result) || ~isfield(result,'backend') || ...
        ~any(strcmp(char(string(result.backend)), ...
        {'continuous_cell_state','scene_parent_v54'}))
...
if ~isfield(result,'global_solver') || ...
        ~isstruct(result.global_solver) || ...
        ~isfield(result.global_solver,'parameters')
    error('cellLatentModel:InvalidContinuousResult', ...
        'Continuous result lacks global_solver.parameters.');
end
if isfield(cfg,'annotation_parent_reranker_manifest') && ...
        ~isempty(cfg.annotation_parent_reranker_manifest) && ...
        (~isfield(result,'annotation_parent_reranker') || ...
         ~isstruct(result.annotation_parent_reranker) || ...
         ~isfield(result.annotation_parent_reranker,'targets_consumed') || ...
         logical(result.annotation_parent_reranker.targets_consumed))
    error('cellLatentModel:InvalidAnnotationParentRerankerResult', ...
        ['Python did not return the required GT-free annotation ' ...
         'parent-reranker audit contract.']);
end
if isfield(cfg,'annotation_budneck_model_manifest') && ...
        ~isempty(cfg.annotation_budneck_model_manifest) && ...
        (~isfield(result,'annotation_budneck_parent') || ...
         ~isstruct(result.annotation_budneck_parent) || ...
         ~isfield(result.annotation_budneck_parent,'targets_consumed') || ...
         logical(result.annotation_budneck_parent.targets_consumed) || ...
         ~isfield(result.annotation_budneck_parent,'bud_null_changed') || ...
         logical(result.annotation_budneck_parent.bud_null_changed) || ...
         ~isfield(result.annotation_budneck_parent,'tracking_changed') || ...
         logical(result.annotation_budneck_parent.tracking_changed))
    error('cellLatentModel:InvalidAnnotationBudneckResult', ...
        ['Python did not return the required target-free, tracking-frozen ' ...
         'bud-neck GT-initialization audit contract.']);
end
assertRuntimeParameters(result.global_solver.parameters, ...
    cfg.global_solver,{'mother_refractory_frames', ...
    'young_mother_frames','beam_size','top_k_candidates'});
if ~isfield(result,'candidate_generation') || ...
        ~isstruct(result.candidate_generation) || ...
        ~isfield(result.candidate_generation,'birth_proposal_parameters')
    error('cellLatentModel:InvalidContinuousResult', ...
        ['Continuous result lacks candidate-generation parameter ' ...
         'provenance.']);
end

assertRuntimeParameters( ...
    result.candidate_generation.birth_proposal_parameters, ...
    cfg.linker_parameters,{'min_parent_age', ...
    'max_parent_centroid_distance','max_parent_contour_distance'});
end

function tf = annotationBudneckEnabled(ctx,param,observations)
tf = false;
try
    tf = isstruct(ctx) && isfield(ctx,'annotationPrediction') && ...
        isstruct(ctx.annotationPrediction) && ...
        isfield(ctx.annotationPrediction,'usePinnedActiveModel') && ...
        logical(ctx.annotationPrediction.usePinnedActiveModel) && ...
        isfield(param,'annotationBudneckModelManifestPath') && ...
        ~isempty(param.annotationBudneckModelManifestPath) && ...
        isfield(param,'annotationBudneckEnabled') && ...
        logical(param.annotationBudneckEnabled) && ...
        isfield(observations,'budneck') && ~isempty(observations.budneck);
catch
    tf = false;
end
end

function assertRuntimeParameters(actual,requested,names)
for i = 1:numel(names)
    name = names{i};
    if ~isfield(requested,name), continue; end
    if ~isfield(actual,name) || ...
            ~isequaln(double(actual.(name)),double(requested.(name)))
        error('cellLatentModel:RuntimeContractMismatch', ...
            ['Python did not honor requested parameter %s ' ...
             '(requested %g).'],name,double(requested.(name)));
    end
end
end
