...
function testExplicitParentageCensorIsExportedWithoutCensoringAppear(testCase)
fixture = benchmarkFixture(testCase);
cleanup = onCleanup(@() removeFolder(fixture.root)); %#ok<NASGU>
roiObj = fixture.classifier.roi(1);
[model,~] = roiObj.loadCellModel('Force',true,'MigrateLegacy',true);
[model,~] = cellModel.setCensoring(model,7,200,3,3, ...
    'Scope','parentage','Reason','ambiguous_parentage');
roiObj.saveCellModel(model,'KeepBackup',false);
approve(testCase,fixture.classifier,roiObj);
classiSave(fixture.classifier);

result = cellLatentModel.exportBenchmarkSnapshot( ...
    fixture.classifier, fixture.split, ...
    'synthetic_explicit_censor_v001', ...
    'ExternalRoot', fixture.externalRoot, ...
    'AllowUnitTestRoot', true);
targets = jsondecode(fileread(result.targets_manifest));
item = itemForRoi(targets.items,'benchmark_roi_a');
targetFile = fullfile(result.directory, ...
    strrep(item.payload_path,'/',filesep));

verifyEqual(testCase,double(h5read(targetFile, ...
    '/lifecycle/birth_class')).',[0 0]);
verifyEqual(testCase,double(h5read(targetFile, ...
    '/lifecycle/birth_observed')).',[0 1], ...
    ['Parentage censorship must not remove independent APPEAR ' ...
     'supervision.']);
verifyEqual(testCase,double(h5read(targetFile, ...
    '/relations/count')),0);
verifyEqual(testCase,double(h5read(targetFile, ...
    '/censoring/track_id')),200);
verifyEqual(testCase,double(h5read(targetFile, ...
    '/censoring/frame_start')),2);
verifyEqual(testCase,double(h5read(targetFile, ...
    '/censoring/scope_flags')),16);
end
