...
p.sceneParentRuntimeManifestPath = strtrim(readChoice( ...
...
if strcmp(p.backend,'causal_composite')
    ...
    p.sceneParentRuntimeManifestPath = resolveArtifactPath( ...
    ...
    if ~isempty(p.sceneParentRuntimeManifestPath) && ...
            ~isfile(p.sceneParentRuntimeManifestPath)
        error('cellLatentModel:MissingSceneParentRuntime', ...
            'The promoted v54 parent-runtime manifest was not found.');
    end
    if ~isempty(p.annotationParentRerankerManifestPath) && ...
            ~isfile(p.annotationParentRerankerManifestPath)
        error('cellLatentModel:MissingAnnotationParentReranker', ...
            'The annotation parent-reranker manifest was not found.');
    end
    if ~isempty(p.annotationParentRerankerManifestPath) && ...
