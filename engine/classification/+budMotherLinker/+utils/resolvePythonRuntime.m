...
%BUDMOTHERLINKER.UTILS.RESOLVEPYTHONRUNTIME Internal external-package runtime.
%
% Runtime discovery is infrastructure, never a classifier static parameter.

if nargin < 1, ctx = struct(); end
runtime = struct('pythonExecutable', '', 'repositoryRoot', '', ...
    'packageSource', '', 'backend', 'local');

environmentNames = {'CELL_LINEAGE_LINKER_PYTHON', 'DETECDIV_PYTHON_EXE'};
...
if isempty(runtime.pythonExecutable)
    error('budMotherLinker:PythonRuntimeUnavailable', ...
        ['No Python runtime is available for cell_lineage_linker. Install ' ...
         'the package in the DetecDiv Python environment.']);
end

repo = strtrim(getenv('CELL_LINEAGE_LINKER_REPO_ROOT'));
if ~hasPackageSource(repo)
    detecdivRoot = fileparts(fileparts(fileparts(fileparts(fileparts( ...
        mfilename('fullpath'))))));
    matlabRoot = fileparts(detecdivRoot);
    candidates = { ...
        fullfile(matlabRoot, 'cell_lineage_linker'), ...
...
tf = ~isempty(root) && isfolder(fullfile(root, 'src', ...
    'cell_lineage_linker'));
end
