...
function testValidationIssueRowsPutBlockingErrorsBeforeWarnings(testCase)
warningIssue = annotationManager.newValidationIssue( ...
    'code','advisory_first','severity','warning', ...
    'component','Tracking','summary','Advisory warning', ...
    'message','A non-blocking warning.','focus_frame',uint32(2));
errorIssue = annotationManager.newValidationIssue( ...
    'code','blocking_second','severity','error', ...
    'component','Parentage','summary','Missing parent track', ...
    'message','A blocking parentage error.','focus_frame',uint32(7));
report = struct('valid',false,'errors',"A blocking parentage error.", ...
    'warnings',"A non-blocking warning.", ...
    'issues',[warningIssue; errorIssue]);

rows = annotationManager.validationIssueRows(report);
verifyEqual(testCase,{rows.severity},{'error','warning'});
verifyEqual(testCase,{rows.code},{'blocking_second','advisory_first'});
verifyEqual(testCase,[rows.issue_index],[2 1], ...
    'Sorting rows must preserve callbacks into the structured issue array.');
end
