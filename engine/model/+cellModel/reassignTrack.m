...
p.addParameter('ConflictPolicy', 'error', @(x) any(strcmpi( ...
    char(string(x)), {'error','replace'})));
p.parse(varargin{:});
fast = p.Results.Fast;
conflictPolicy = lower(char(string(p.Results.ConflictPolicy)));
...
if ~isempty(conflictRows)
    ...
    if strcmp(conflictPolicy, 'error')
    ...
    % Keep the conflicting contours/instances, but detach their stale track
    % identity so the selected biological identity can replace them.
    model.instances.track_id(conflictRows) = uint64(0);
end
...
report = struct('status', 'ok', 'scope', scope, ...
    ...
    'conflict_frames', double(unique(model.instances.frame(conflictRows))).', ...
    'conflicts_replaced', numel(conflictRows), ...
