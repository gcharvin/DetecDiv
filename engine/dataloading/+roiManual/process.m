...
% roiManual.process  Open manual ROI editing on the selected FOVs.
    ...
    if ~isfield(ctx, 'shallow') || isempty(ctx.shallow) || ~isa(ctx.shallow, 'shallow')
        error('roiManual.process:ProjectRequired', 'Manual ROI editing requires a shallow project context.');
    end
