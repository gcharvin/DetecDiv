function out = identifyROIsLocal(varargin)
...
%img = rgb2gray(img);
  ...
  % V??rifier que l'entr??e est une image valide
    if ndims(img) == 2
    ...
    elseif ndims(img) == 3 && size(img, 3) == 3
        ...
        % img_gray = mean(img, 3);  % mais moins fid??le ?? la perception visuelle
    else
       warning('Image non support??e : doit ??tre 2D (grayscale) ou 3D avec 3 canaux (RGB)');
    end
