...
% The reason picker uses LISTDLG (rather than UIFIGURE-only UICONFIRM) so
% it works consistently from both the Score UI and classic MATLAB figures.
...
reasonChoices = {'Clearly truncated at ROI boundary', ...
    'Ambiguous identity','Ambiguous parentage', ...
    'Unusable segmentation','Other'};
[reasonIndex, accepted] = listdlg( ...
    'PromptString', { ...
        'Choose a documented reason.', ...
        ['A cell that merely touches the ROI edge must remain usable; ' ...
         'choose boundary truncation only when the cell or its mask is ' ...
         'visibly cut.']}, ...
    'SelectionMode', 'single', 'ListString', reasonChoices, ...
    'InitialValue', 1, 'ListSize', [430 155], ...
    'Name', 'Censoring reason');
if ~accepted || isempty(reasonIndex), return; end
reasonChoice = reasonChoices{reasonIndex};
