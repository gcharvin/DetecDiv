...
function contextMenuDuplicateProcessFcn(src,event,arg,str) %#ok<INUSD>
    ...
    if strcmp(str,'Projectprocess')
        ...
        processDataGUI(shallowObj, proc);
            ...
            function pNode = createRecentPipelineTreeNode(parentNode, pipelineJson, pth)
                pNode = [];
                ...
                cm=uicontextmenu(app.DetecDivUIFigure);
                m = uimenu(cm,'Text','Open pipeline...');
                m.MenuSelectedFcn = @(~,~)app.openRecentPipelineCallback(pipelineJson);
                pNode=uitreenode(parentNode,'Text',label,'Tag','PipelineRecent','UserData',pipelineJson, ...
                    'ContextMenu',cm,'Icon',fullfile(pth,'pipeline.png'));
            end
            ...
            function [runMode, runStatus] = summarizePipelineRun(runObj)
                ...
                try
                    if isstruct(runObj.ctx) && isfield(runObj.ctx,'hub') && isstruct(runObj.ctx.hub)
                        runMode = 'hub';
                        if strcmp(runStatus, 'unknown') && isfield(runObj.ctx.hub,'status') && ~isempty(runObj.ctx.hub.status)
            ...
            function shallowObj = getProjectByIndex(projIdx)
                shallowObj = [];
                if projIdx > numel(app.Data.Project)
                    return;
                end
            ...
            function openProjectPipelineRunInspector(projIdx, runIdx)
            ...
            function executeProjectPipelineRun(projIdx, runIdx)
                ...
                runObj = backfillRunPipelineRef(runObj, pipeObj, shallowObj);
            ...
            function jobId = localRunHubJobId(runObj)
                ...
                try
                    ...
                    if isempty(jobId) && isstruct(runObj.ctx) && isfield(runObj.ctx, 'run') && isstruct(runObj.ctx.run) && ...
                            isfield(runObj.ctx.run, 'control') && isstruct(runObj.ctx.run.control) && ...
                            isfield(runObj.ctx.run.control, 'jobId') && ~isempty(runObj.ctx.run.control.jobId)
                        jobId = char(string(runObj.ctx.run.control.jobId));
                    end
                catch
                end
            end
...
for k = 1:numel(procList)
    ...
    methods (Access = private)
        ...
        function text = describePipelineTemplate(app, pipeObj) %#ok<INUSD>
            text = '';
            if isempty(pipeObj) || ~isa(pipeObj, 'pipeline')
                return;
            end
            ...
            for iNode = 1:numel(nodes)
                ...
                nodePkg = '';
                try
                    if isfield(node,'id') && ~isempty(node.id), nodeId = char(string(node.id)); end
                    if isfield(node,'type') && ~isempty(node.type), nodeType = char(string(node.type)); end
                    if isfield(node,'pkg') && ~isempty(node.pkg), nodePkg = char(string(node.pkg)); end
                catch
                end
                if isempty(nodeId)
                    nodeId = ['node_' num2str(iNode)];
                end
                parts = {nodeId};
                if ~isempty(nodeType), parts{end+1} = ['[' nodeType ']']; end %#ok<AGROW>
                if ~isempty(nodePkg), parts{end+1} = ['package=' nodePkg]; end %#ok<AGROW>
                text = [text '- ' strjoin(parts, ' ') newline]; %#ok<AGROW>
            end
        end
        ...
        function openProjectPipelineTemplate(app, shallowObj)
            if isempty(shallowObj) || ~isa(shallowObj, 'shallow')
                return;
            end
            ...
            if isempty(pipeObj) || ~isa(pipeObj, 'pipeline')
                ...
                if ~found
                    uialert(app.DetecDivUIFigure, 'No pipeline template could be resolved for this project.', 'Open project pipeline', 'Icon', 'warning');
                    return;
                end
            end
            app.openPipelineWithContext(pipeObj);
        ...
        function createProjectPipelineTemplate(app, shallowObj)
            if isempty(shallowObj) || ~isa(shallowObj, 'shallow')
                return;
            end
            projectRoot = fullfile(char(string(shallowObj.io.path)), char(string(shallowObj.io.file)));
            if isempty(projectRoot) || exist(projectRoot, 'dir') ~= 7
                uialert(app.DetecDivUIFigure, 'Project folder is not available; cannot create a project pipeline template.', 'New pipeline template', 'Icon', 'warning');
                return;
            end

            defaultName = [char(string(shallowObj.io.file)) '_pipeline'];
            answer = inputdlg({'Pipeline template name:'}, 'New pipeline template', [1 60], {defaultName});
            if isempty(answer)
                return;
            end
            ...
            try
                pipeObj = pipelineNew('path', projectRoot, 'name', pipeName, 'workspace', true);
            catch ME
                uialert(app.DetecDivUIFigure, ME.message, 'New pipeline template', 'Icon', 'error');
                return;
            end
            if isempty(pipeObj)
                return;
            ...
            try
                rawPath = app.deriveRawDataPathFromProject(shallowObj);
                if ~isempty(rawPath)
                    app.populateDefaultDataLoaderPath(pipeObj, rawPath);
                end
                app.setProjectDefaultPipelineRef(shallowObj, pipeObj);
                pipelineSave(pipeObj);
                app.publishPipelineObjectToWorkspace(pipeObj);
                app.saveShallowProjectWithProgress(shallowObj, 'Saving project pipeline template...', 'shallowObj');
            catch ME
                warning('detecdiv:ProjectPipelineTemplate', '%s', ME.message);
            end
        ...
        function openLatestProjectRunOrPipeline(app, shallowObj)
            if isempty(shallowObj) || ~isa(shallowObj, 'shallow')
                return;
            end
        ...
        function runObj = latestProjectPipelineRun(app, shallowObj) %#ok<INUSD>
            runObj = [];
            try
                runs = shallowObj.processing.pipelineRun;
            catch
            ...
            for i = 1:numel(runs)
                ...
                try
                    txt = '';
                    if isprop(runs(i), 'updatedAt') && ~isempty(runs(i).updatedAt)
                        txt = char(string(runs(i).updatedAt));
                    elseif isprop(runs(i), 'createdAt') && ~isempty(runs(i).createdAt)
                        txt = char(string(runs(i).createdAt));
                    end
                    if ~isempty(txt)
                        dt = datetime(txt);
                        if ~isnat(dt)
        ...
        function changed = replaceProjectDefaultRoiProducerNode(app, shallowObj, nodeType, params)
            ...
            if strcmpi(char(string(nodeType)), 'roiTracked')
                changed = app.ensureProjectTrackedPipelineNode(shallowObj, params);
                return;
            end
            ...
            if strcmpi(newNode.type, 'roiTracked')
                [pipeObj.nodes, ~] = app.ensureTrackedMaskSupportNodesInTemplate(pipeObj.nodes);
                nodes = pipeObj.nodes;
            end
            ...
            for iNode = 1:numel(nodes)
                ...
                elseif strcmp(t, 'classifier') && isempty(maskSourceId)
                    ...
                    if isempty(outs)
                        try
                            ctr = pipelineNodeContract(nodes(iNode));
            ...
            if strcmpi(newNode.type, 'roiTracked') && ~isempty(maskSourceId) && ~isempty(extractId)
                if ~isempty(loaderId)
                ...
                normEdges(end+1) = struct('from', extractId, 'to', maskSourceId, 'fromPort', 'roiList', 'toPort', 'roiList', 'condition', ''); %#ok<AGROW>
                normEdges(end+1) = struct('from', newNode.id, 'to', maskSourceId, 'fromPort', 'roiList', 'toPort', 'roiList', 'condition', ''); %#ok<AGROW>
        ...
        function [nodes, changed] = ensureTrackedMaskSupportNodesInTemplate(app, nodes)
            changed = false;
            if nargin < 2 || isempty(nodes)
                nodes = struct([]);
            end

            exIdx = app.findNodeIndexInTemplate(nodes, 'roiExtract');
            if isempty(exIdx)
                exNode = app.buildDefaultTemplateExtractNode();
                ...
                for iNode = 1:numel(nodes)
                    ...
                    if any(strcmp(t, {'roiidentify','roipattern','roimanual','roigrid','roitracked'}))
                        insertPos = iNode + 1;
                    end
                end
            ...
            maskIdx = app.findMaskClassifierNodeIndexInTemplate(nodes);
            if isempty(maskIdx)
                maskNode = app.buildDefaultMaskClassifierTemplateNode();
                if isempty(exIdx)
                    insertPos = numel(nodes) + 1;
                else
                    insertPos = exIdx + 1;
        ...
        function idx = findNodeIndexInTemplate(app, nodes, typeName) %#ok<INUSD>
            idx = [];
            for iNode = 1:numel(nodes)
                if isfield(nodes(iNode),'type') && strcmpi(char(string(nodes(iNode).type)), char(string(typeName)))
                    idx = iNode;
                    return;
                end
            end
        ...
        function idx = findMaskClassifierNodeIndexInTemplate(app, nodes)
            idx = [];
            for iNode = 1:numel(nodes)
                if ~isfield(nodes(iNode),'type') || ~strcmpi(char(string(nodes(iNode).type)), 'classifier')
                    continue;
                end
                if app.templateNodeProducesMasks(nodes(iNode))
                    idx = iNode;
                    return;
                end
            end
        ...
        function tf = templateNodeProducesMasks(app, node) %#ok<INUSD>
            tf = false;
            outs = {};
        ...
        function node = buildDefaultTemplateExtractNode(app) %#ok<INUSD>
            defaults = roiExtract.setparam(struct());
            node = struct('id','roiextract_1','name','roiextract_1','type','roiExtract','func','roiExtract.process','gui','roiExtract.ui','guiMode','replace','paramRequired',{{}},'pkg','','params',defaults,'inputs',{{'roiList'}},'outputs',{{'roiList','channels'}},'enabled',true,'status','','layout',[70 10 20 10]);
        end

        function node = buildDefaultMaskClassifierTemplateNode(app) %#ok<INUSD>
            params = struct('pkg','cellposesam','outputName','tracked_masks');
            node = struct('id','classifier_mask_1','name','classifier_mask_1','type','classifier','func','cellposesam.classify','gui','classifierGUI','guiMode','replace','paramRequired',{{'pkg'}},'pkg','cellposesam','params',params,'inputs',{{'roiList'}},'outputs',{{'roiList','masks'}},'enabled',true,'status','','layout',[95 10 24 10]);
        end
        ...
        function baseType = getTrackedBaseRoiType(app, pipeObj) %#ok<INUSD>
            baseType = '';
            fallbackType = '';
            for iNode = 1:numel(pipeObj.nodes)
                node = pipeObj.nodes(iNode);
                ...
                if ~any(strcmp(t, {'roipattern','roiidentify','roimanual','roigrid'}))
                    continue;
                end
                canon = t;
                if any(strcmp(t, {'roipattern','roiidentify'}))
                    canon = 'roipattern';
                end
            ...
            if isempty(baseType)
                if isempty(fallbackType)
                    ...
                    baseType = fallbackType;
                end
            end
        end

        function changed = ensureProjectTrackedPipelineNode(app, shallowObj, params)
            changed = false;
            ...
            nodes = pipeObj.nodes;
            sourceType = app.getTrackedBaseRoiType(pipeObj);

            if isempty(app.findNodeIndexInTemplate(nodes, sourceType))
                srcNode = app.buildDefaultRoiProducerNode(sourceType, struct(), [40 10 20 10]);
                if isempty(nodes)
                    nodes = srcNode;
                else
            ...
            trackedIdx = app.findNodeIndexInTemplate(nodes, 'roiTracked');
            if isempty(trackedIdx)
                trackedNode = app.buildDefaultRoiProducerNode('roiTracked', params, [58 10 20 10]);
                insertPos = numel(nodes) + 1;
                for iNode = 1:numel(nodes)
                    t = '';
                    if isfield(nodes(iNode),'type')
                ...
                nodes = [nodes(1:insertPos-1) trackedNode nodes(insertPos:end)]; %#ok<AGROW>
                changed = true;
            elseif ~isempty(fieldnames(params))
                nodes(trackedIdx).params = params;
                changed = true;
            end

            [nodes, supportChanged] = app.ensureTrackedMaskSupportNodesInTemplate(nodes);
            ...
            for iNode = 1:numel(nodes)
                ...
                if strcmp(t,'dataloader') && isempty(loaderId)
                    loaderId = char(string(node.id));
                elseif any(strcmp(t, {'roipattern','roiidentify','roimanual','roigrid'}))
                    canon = t;
                    if any(strcmp(t, {'roipattern','roiidentify'}))
                        canon = 'roipattern';
                    end
                    ...
                    if enabled && strcmp(canon, sourceType) && isempty(sourceId)
                        sourceId = char(string(node.id));
                    end
                elseif strcmp(t,'roitracked') && isempty(trackedId)
                    trackedId = char(string(node.id));
                elseif strcmp(t,'roiextract') && isempty(extractId)
                    extractId = char(string(node.id));
                elseif strcmp(t,'classifier') && isempty(maskSourceId) && app.templateNodeProducesMasks(node)
                    maskSourceId = char(string(node.id));
                end
            end

            if isempty(sourceId)
                for iNode = 1:numel(nodes)
                    if isfield(nodes(iNode),'type') && any(strcmpi(char(string(nodes(iNode).type)), {'roiPattern','roiIdentify','roiManual','roiGrid'}))
                        sourceId = char(string(nodes(iNode).id));
                        break;
                    end
                end
            end

            keep = true(1, numel(pipeObj.edges));
            rebuildIds = unique([{sourceId}, {trackedId}, {extractId}, {maskSourceId}, {loaderId}]);
            rebuildIds = rebuildIds(~cellfun(@isempty, rebuildIds));
            for iEdge = 1:numel(pipeObj.edges)
                e = pipeObj.edges(iEdge);
                fromId = '';
                toId = '';
                if isfield(e,'from'), fromId = char(string(e.from)); end
                if isfield(e,'to'), toId = char(string(e.to)); end
                if any(strcmp(rebuildIds, fromId)) || any(strcmp(rebuildIds, toId))
                    keep(iEdge) = false;
                end
            end
            normEdges = pipeObj.edges(keep);

            if ~isempty(loaderId) && ~isempty(sourceId)
                normEdges(end+1) = struct('from', loaderId, 'to', sourceId, 'fromPort', 'images', 'toPort', 'images', 'condition', ''); %#ok<AGROW>
            end
            if ~isempty(sourceId) && ~isempty(extractId)
                normEdges(end+1) = struct('from', sourceId, 'to', extractId, 'fromPort', 'roiList', 'toPort', 'roiList', 'condition', ''); %#ok<AGROW>
            end
            if ~isempty(extractId) && ~isempty(maskSourceId)
                normEdges(end+1) = struct('from', extractId, 'to', maskSourceId, 'fromPort', 'roiList', 'toPort', 'roiList', 'condition', ''); %#ok<AGROW>
            end
            if ~isempty(sourceId) && ~isempty(trackedId)
                normEdges(end+1) = struct('from', sourceId, 'to', trackedId, 'fromPort', 'roiList', 'toPort', 'roiList', 'condition', ''); %#ok<AGROW>
            end
            if ~isempty(maskSourceId) && ~isempty(trackedId)
                normEdges(end+1) = struct('from', maskSourceId, 'to', trackedId, 'fromPort', 'masks', 'toPort', 'masks', 'condition', ''); %#ok<AGROW>
            end
            pipeObj.edges = normEdges;
            ...
            pipeObj.runProfiles.roiWorkflow.mode = 'tracked';

            for iNode = 1:numel(pipeObj.nodes)
                if ~isfield(pipeObj.nodes(iNode),'type')
                    continue;
                end
                t = lower(char(string(pipeObj.nodes(iNode).type)));
                if any(strcmp(t, {'roipattern','roiidentify'}))
                    pipeObj.nodes(iNode).enabled = strcmp(sourceType, 'roipattern');
                elseif strcmp(t, 'roimanual')
                    pipeObj.nodes(iNode).enabled = strcmp(sourceType, 'roimanual');
                elseif strcmp(t, 'roigrid')
                    pipeObj.nodes(iNode).enabled = strcmp(sourceType, 'roigrid');
                elseif strcmp(t, 'roitracked')
                    pipeObj.nodes(iNode).enabled = true;
                end
            end
        ...
        function changed = setProjectDefaultPipelineRoiMode(app, shallowObj, modeName)
            ...
            if strcmp(modeName, 'tracked')
                changed = app.ensureProjectTrackedPipelineNode(shallowObj, struct()) || changed;
            end
            ...
            trackedBaseType = '';
            if strcmp(modeName, 'tracked')
                trackedBaseType = app.getTrackedBaseRoiType(pipeObj);
            end
            ...
            for iNode = 1:numel(pipeObj.nodes)
                ...
                if strcmpi(char(string(node.type)), 'roiidentify') || strcmpi(char(string(node.type)), 'roiPattern')
                    if strcmp(modeName, 'tracked')
                        newEnabled = strcmp(trackedBaseType, 'roipattern');
                    else
                        newEnabled = strcmp(modeName, 'pattern');
                    end
                elseif strcmpi(char(string(node.type)), 'roiManual')
                    if strcmp(modeName, 'tracked')
                        newEnabled = strcmp(trackedBaseType, 'roimanual');
                    else
                ...
                elseif strcmpi(char(string(node.type)), 'roiTracked')
                    ...
                    if strcmp(modeName, 'tracked')
                        newEnabled = strcmp(trackedBaseType, 'roigrid');
                    else
   ...
   % [guiFolder, ~, ~] = fileparts(thisAppFile);
    ...
    methods (Access = private)
        ...
        function TreeSelectionChanged(app, event)
            ...
            if numel(selectedNodes.Tag)==0 & any(strcmp(selectedNodes.Text, {'Independent Classifiers','Classifiers'}))
                app.ProjectsPanel.Title='';
                app.ProjectInformationLabel.Text='';
            end
                    ...
                    if isfield(shallowObj.processing,'pipelineRun') && ~isempty(shallowObj.processing.pipelineRun)
                        ...
                        for ir = 1:numel(runs)
                            if isprop(runs(ir),'pipelineRef') && isstruct(runs(ir).pipelineRef)
                                if isfield(runs(ir).pipelineRef,'id') && ~isempty(runs(ir).pipelineRef.id)
                                    runTemplates{end+1} = char(string(runs(ir).pipelineRef.id)); %#ok<AGROW>
                                elseif isfield(runs(ir).pipelineRef,'path') && ~isempty(runs(ir).pipelineRef.path)
                                    [~, nm] = fileparts(char(string(runs(ir).pipelineRef.path)));
                                    runTemplates{end+1} = nm; %#ok<AGROW>
                                end
                            end
                        end
                    end
                    runTemplates = unique(runTemplates, 'stable');
                    if ~isempty(runTemplates)
                        t=[t 'Pipeline templates used in project: ' num2str(numel(runTemplates)) newline];
                        t=[t strjoin(runTemplates, ', ') newline newline];
                    end
                catch
                end
            ...
            if strcmp(selectedNodes.Tag,'Pipeline')
                ...
                if idx <= numel(app.Data.Pipeline)
                    ...
                    t=[t 'Connections: ' num2str(nEdges) newline newline];
                    t=[t app.describePipelineTemplate(pipeObj)];
                    app.ProjectInformationLabel.Text=t;
                end
            end

            if strcmp(selectedNodes.Tag,'PipelineRecent')
                app.ProjectsPanel.Title='Recent pipeline';
                app.OpenButton.Visible='on';
                app.OpenButton.Text='Open Pipeline...';
                app.ProcessdataButton.Visible='off';

                pipelineJson = char(string(selectedNodes.UserData));
                t='';
                t=[t 'Pipeline path: ' newline pipelineJson newline newline];
                if exist(pipelineJson, 'file') ~= 2
                    t=[t 'Status: missing file' newline];
                else
                    try
                        [pipeObj, msg] = pipelineLoad(pipelineJson);
                        if isempty(pipeObj)
                            t=[t 'Status: cannot load pipeline' newline char(string(msg)) newline];
                        else
                            t=[t 'Name: ' char(string(pipeObj.strid)) newline];
                            t=[t 'Nodes: ' num2str(numel(pipeObj.nodes)) newline];
                            t=[t 'Connections: ' num2str(numel(pipeObj.edges)) newline newline];
                            t=[t app.describePipelineTemplate(pipeObj)];
                        end
                    catch ME
                        t=[t 'Status: cannot inspect pipeline' newline ME.message newline];
                    end
                end
                app.ProjectInformationLabel.Text=t;
            end
                ...
                if isfield(shallowObj.processing,'pipelineRun') && cc(2) <= numel(shallowObj.processing.pipelineRun)
                    ...
                    try
                        if isstruct(runObj.ctx) && isfield(runObj.ctx,'io') && isstruct(runObj.ctx.io)
                            if isfield(runObj.ctx.io,'existingPolicy') && ~isempty(runObj.ctx.io.existingPolicy)
                                t=[t 'Existing policy: ' char(string(runObj.ctx.io.existingPolicy)) newline];
                            end
                            if isfield(runObj.ctx.io,'cachePolicy') && ~isempty(runObj.ctx.io.cachePolicy)
                                t=[t 'Cache policy: ' char(string(runObj.ctx.io.cachePolicy)) newline];
                            end
                        end
                    catch
                    end
                    ...
                    try
                        if isprop(runObj,'createdAt') && ~isempty(runObj.createdAt)
                            t=[t 'Created: ' char(string(runObj.createdAt)) newline];
                        end
        ...
        function AddclassifierButtonPushed(app, event)
            ...
            if strcmp(app.Tree.SelectedNodes.Tag,'Project')
                ...
                shallowObj=evalin('base',proj);
                app.openLatestProjectRunOrPipeline(shallowObj);
                return;
            end
        end
        ...
        function UpdaterawdatapathButtonPushed(app, event)
            ...
            proj=app.Data.Project{i};
            shallowObj=evalin('base',proj);
            app.createProjectPipelineTemplate(shallowObj);
            TreeSelectionChanged(app, event)
            gatherVarsFromWorkspace(app);
            displayNodes(app);
...
% Menu selected function: OpenprojectindependentclassifierMenu
    ...
    close(d);
        ...
        function ClassifydataButtonPushed(app, event)
            ...
            shallowObj=evalin('base',proj);
            uialert(app.DetecDivUIFigure, ...
                ['Project-level classification now goes through pipeline templates.' newline ...
                 'Open the project pipeline, adjust the run parameters, then run it from pipeline2.'], ...
                'Use pipelines', 'Icon', 'info');
        end
        ...
        function AddprocessorButtonPushed(app, event)
            ...
            if strcmp(app.Tree.SelectedNodes.Tag,'Project')
                uialert(app.DetecDivUIFigure, ...
                    ['Project-level processors are now expected to be configured inside a pipeline template.' newline ...
                     'Create or open the project pipeline and add the processor there.'], ...
                    'Use pipelines', 'Icon', 'info');
                return;
            end
        end
    ...
    try
        ...
        if strcmp(str,'PipelineRecent')
            d.Message = 'Loading recent pipeline...';
            pipelineJson = char(string(arg));
            try
                close(d);
            catch
            end
            app.openRecentPipelineCallback(pipelineJson);
            return;
        end
        ...
        if strcmp(str,'ProjectpipelineRun')
            ...
            if isempty(pipeObj) && isprop(runObj,'templatePath') && ~isempty(runObj.templatePath)
                [pipeObj, ~] = pipelineLoad(runObj.templatePath);
            end
            if isempty(pipeObj)
                try
                    [foundDefault, defaultPipe] = app.getProjectDefaultPipelineObject(shallowObj);
    ...
    try
        ...
        function InspectRunButtonPushed(app, event)
            if numel(app.Tree.SelectedNodes)==0
                return;
            end
