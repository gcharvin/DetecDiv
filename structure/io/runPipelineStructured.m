...
function ctx = executeClassifierNode(node, ctx)
    ...
    selectedChannels = [];
    if any(strcmpi(char(string(pkgName)), ...
            {'trackastra','cellLatentTracker'}))
        selectedChannels = compactChannelParams(p, ...
    ...
    elseif strcmpi(char(string(pkgName)),'cellLatentModel')
        selectedChannels = compactChannelParams(p, ...
            {'instanceChannelName','trackChannelName', ...
             'brightfieldChannelName','gfpChannelName', ...
