...
model = struct( ...
    ...
    'modelUpdatePolicy', textField(params, 'modelUpdatePolicy'), ...
    'releaseId', textField(params, 'resolvedModelReleaseId'), ...
    'releaseManifestPath', textField(params, ...
    ...
    'sceneParentRuntimeManifestPath', ...
        status.sceneParentRuntimeManifestPath.path, ...
    'annotationParentRerankerManifestPath', ...
        status.annotationParentRerankerManifestPath.path, ...
    'annotationBudneckModelManifestPath', ...
        status.annotationBudneckModelManifestPath.path, ...
...
switch backend
    case 'causal_composite'
        ...
        if ~isempty(textField(params, 'sceneParentRuntimeManifestPath'))
            issues = requireArtifact(issues, status, ...
                'sceneParentRuntimeManifestPath', ...
                'The active v54 parent runtime is missing.');
        end
        if ~isempty(textField(params, ...
                'annotationParentRerankerManifestPath'))
            issues = requireArtifact(issues, status, ...
                'annotationParentRerankerManifestPath', ...
                'The active annotation parent reranker is missing.');
        end
        if ~isempty(textField(params, ...
                'annotationBudneckModelManifestPath'))
            issues = requireArtifact(issues, status, ...
                'annotationBudneckModelManifestPath', ...
                'The active annotation bud-neck transformer is missing.');
        end
...
markerIdentity = selectorOverride(override, 'budneckMarkerIdentity');
if isempty(markerIdentity)
    markerIdentity = textField(baseParams, 'budneckMarkerIdentity');
end
if isempty(markerIdentity), markerIdentity = 'MYO1'; end
params.budneckMarkerIdentity = markerIdentity;
params.annotationBudneckEnabled = false;
if isstruct(override) && isscalar(override) && ...
        isfield(override, 'annotationBudneckEnabled')
    try
        params.annotationBudneckEnabled = logical( ...
            override.annotationBudneckEnabled);
    catch
    end
elseif isstruct(override) && isscalar(override) && ...
        isfield(override, 'budneckChannelName')
    params.annotationBudneckEnabled = ~isempty(budneck) && ...
        ~isNone(selectorOverride(override, 'budneckChannelName'));
end
...
try
    ...
    model.provenance.last_prediction_model_release_id = ...
        report.model.releaseId;
    model.provenance.last_prediction_model_release_manifest = ...
        report.model.releaseManifestPath;
