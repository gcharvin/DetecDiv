        ...
        case 'processor'
            ...
            elseif strcmp(p, 'singlecelloscillations') || contains(f, 'singlecelloscillations')
                ...
                resources.out = [ ...
                    ...
                    resourceDef('dataSeries', 'oscillation_cycles', 'osc_normalized_cycles', 'normalizedCyclesOutputName', 'dataSeries', 'normalizedCyclesOutputName', false, 'roiDataSeries'), ...
    ...
    if strcmp(nodeType, 'classifier')
        switch pkgName
            ...
            case 'celllatenttracker'
                contract.parameters.static = unique([contract.parameters.static cellLatentTrackerExecutionStaticKeys()], 'stable');
                contract.requirements.roi.channelsMin = max(contract.requirements.roi.channelsMin, 1);
                contract.capabilities.outputsMasks = false;
                contract.capabilities.outputsChannels = true;
                contract.capabilities.roiMasks = false;
                ...
                contract.binding.selectorKeys = {'imageChannelName','instanceChannelName'};
                contract.binding.resolveAt = 'run';
                contract.binding.minCount = 1;
                contract.binding.exactCount = [];
                contract.selectors.channelParam = 'imageChannelName';
                contract.selectors.outputNameParam = 'outputName';
                contract.out = [ ...
                    portDef('roiList', 'roiList', true, 'edge'), ...
                ...
                contract.resources.in = [ ...
                    resourceDef('channel', 'brightfield_image', 'imageChannelName', 'imageChannelName', 'channels', 'imageChannelName', false, ''), ...
                    resourceDef('channel', 'mask_roi_image', 'instanceChannelName', 'instanceChannelName', 'channels', 'instanceChannelName', true, '') ...
                    ];
                contract.resources.out = resourceDef('channel', 'tracking', 'tracklets', 'outputName', 'channels', 'outputName', false, 'roiChannel');
                contract.summary = ['Latent EDGE/APPEAR/END tracker: consumes frame-local instances and optional ' ...
                    'brightfield, then writes stable track IDs. It does not invoke CellposeSAM or Trackastra.'];
            case 'celllatentmodel'
                backend = normalizeOutputMode(getNestedParam(node, {'backend'}, 'legacy'), ...
                    {'causal_composite','legacy','temporal_lineage','continuous_cell_state'}, 'legacy');
                temporalVariant = normalizeOutputMode(getNestedParam(node, {'temporalVariant'}, 'temporal_geometry'), ...
                    {'temporal_geometry','all_observed'}, 'temporal_geometry');
                stateUpdateMode = normalizeOutputMode(getNestedParam(node, {'stateUpdateMode'}, 'none'), ...
                    {'none','promoted_frozen_bf'}, 'none');
                contract.parameters.static = unique([contract.parameters.static cellLatentModelExecutionStaticKeys()], 'stable');
                contract.requirements.roi.channelsMin = max(contract.requirements.roi.channelsMin, 1);
                contract.capabilities.outputsMasks = false;
                contract.capabilities.outputsChannels = false;
                contract.capabilities.roiMasks = false;
                ...
                contract.binding.minCount = 1;
                contract.binding.exactCount = [];
                contract.binding.defaultCount = [];
                contract.selectors.channelParam = 'trackChannelName';
                contract.selectors.channelsParam = '';
                contract.selectors.framesParam = '';
                contract.selectors.outputNameParam = 'outputFamilyName';
                contract.out = [ ...
                    portDef('roiList', 'roiList', true, 'edge'), ...
                    portDef('dataSeries', 'dataSeriesSet', true, 'edge'), ...
                    portDef('objects', 'cellObjectSet', true, 'edge') ...
                    ];
                contract.resources.in = resourceDef('channel', 'mask_roi_image', ...
                    'tracked_masks', 'trackChannelName', 'channels', ...
                    'trackChannelName', true, '');
                if strcmp(backend,'causal_composite')
                    contract.capabilities.outputsChannels = true;
                    contract.capabilities.roiChannels = true;
                    contract.binding.selectorKeys = {'instanceChannelName', ...
                        'brightfieldChannelName','nucleusChannelName', ...
                        'budneckChannelName'};
                    contract.selectors.channelParam = 'instanceChannelName';
                    contract.resources.in = [ ...
                        resourceDef('channel','mask_roi_image', ...
                            'frame_local_instances','instanceChannelName', ...
                            'channels','instanceChannelName',true,''), ...
                        resourceDef('channel','brightfield_image', ...
                            'brightfield_image','brightfieldChannelName', ...
                            'channels','brightfieldChannelName', ...
                            strcmp(stateUpdateMode,'promoted_frozen_bf'),''), ...
                        resourceDef('channel','division_nucleus_fluorescence', ...
                            'division_nucleus_fluorescence','nucleusChannelName', ...
                            'channels','nucleusChannelName',false,''), ...
                        resourceDef('channel','bud_neck_fluorescence', ...
                            'bud_neck_fluorescence','budneckChannelName', ...
                            'channels','budneckChannelName',false,'') ...
                        ];
                    contract.out(end+1) = portDef( ...
                        'channels','channelSet',true,'edge');
                    contract.summary = ['Composite latent-cell classifier: ' ...
                        'consumes frame-local instances and typed raw observations, ' ...
                        'then writes predicted stable tracks, mother/NULL lineage, ' ...
                        'and optional biological states.'];
                elseif strcmp(backend, 'legacy')
                    contract.binding.selectorKeys = {'trackChannelName','gfpChannelName'};
                    contract.resources.in(end+1) = resourceDef('channel', 'legacy_gfp_fluorescence', ...
                        'legacy_gfp_fluorescence', 'gfpChannelName', 'channels', ...
                        'gfpChannelName', false, '');
                    contract.summary = ['Cell latent relation classifier (legacy backend): consumes tracked masks and ' ...
                        'optional generic GFP without inferring a temporal biological role, then writes a lineage family.'];
                elseif strcmp(backend, 'continuous_cell_state')
                    contract.binding.selectorKeys = {'trackChannelName','brightfieldChannelName', ...
                        'nucleusChannelName','budneckChannelName'};
                    contract.resources.in(end+1) = resourceDef('channel', 'brightfield_image', ...
                        'brightfield_image', 'brightfieldChannelName', 'channels', ...
                        'brightfieldChannelName', false, '');
                    contract.resources.in(end+1) = resourceDef('channel', 'division_nucleus_fluorescence', ...
                        'division_nucleus_fluorescence', 'nucleusChannelName', 'channels', ...
                        'nucleusChannelName', false, '');
                    contract.resources.in(end+1) = resourceDef('channel', 'bud_neck_fluorescence', ...
                        'bud_neck_fluorescence', 'budneckChannelName', 'channels', ...
                        'budneckChannelName', false, '');
                    contract.summary = ['Cell latent continuous-state classifier: consumes stable tracked masks plus ' ...
                        'explicit optional brightfield, division/nucleus, and bud-neck observations, writes a lineage ' ...
                        'family, and persists per-track/frame biological states in a versioned sidecar.'];
                elseif strcmp(temporalVariant, 'all_observed')
                    contract.binding.selectorKeys = {'trackChannelName','nucleusChannelName','budneckChannelName'};
                    contract.resources.in(end+1) = resourceDef('channel', 'division_nucleus_fluorescence', ...
                        'division_nucleus_fluorescence', 'nucleusChannelName', 'channels', ...
                        'nucleusChannelName', false, '');
                    contract.resources.in(end+1) = resourceDef('channel', 'bud_neck_fluorescence', ...
                        'bud_neck_fluorescence', 'budneckChannelName', 'channels', ...
                        'budneckChannelName', false, '');
                    contract.summary = ['Cell latent temporal-lineage classifier (all observed): consumes tracked masks ' ...
                        'plus explicit division/nucleus and bud-neck fluorescence, never inferred from generic GFP, ' ...
                        'then writes a lineage family.'];
                else
                    contract.binding.selectorKeys = {'trackChannelName'};
                    contract.summary = ['Cell latent temporal-lineage classifier (geometry): consumes tracked masks ' ...
                        'and writes a lineage family.'];
                end
                lineageOutputs = [ ...
                    resourceDef('dataSeries', 'lineage', ...
                        'cell_latent_lineage', 'outputFamilyName', 'dataSeries', ...
                        'outputFamilyName', false, 'roiDataSeries'), ...
                    resourceDef('cellModel', 'cellular_objects', ...
                        'cell_latent_objects', 'outputFamilyName', 'objects', ...
                        'outputFamilyName', false, 'roiCellModel') ...
                    ];
                if strcmp(backend,'causal_composite')
                    contract.resources.out = [ ...
                        resourceDef('channel','tracking','predicted_tracks', ...
                            'outputTrackChannelName','channels', ...
                            'outputTrackChannelName',false,'roiChannel'), ...
                        lineageOutputs ...
                        ];
                else
                    contract.resources.out = lineageOutputs;
                end
            ...
            case 'cnn'
                contract.parameters.static = unique([contract.parameters.static cnnExecutionStaticKeys()], 'stable');
                contract.binding.mode = 'singleChannel';
                contract.binding.exactCount = 1;
                contract.binding.resolveAt = 'design';
                contract.capabilities.outputsMasks = false;
                ...
                contract.resources.out = resourceDef('dataSeries', ...
                    'classification', 'dataSeries', 'outputName', ...
                    'dataSeries', 'outputName', false, 'roiDataSeries');
                contract.summary = ['CNN image classifier: consumes microscopy images and writes ' ...
                    'a PRED class-score dataseries. Temporal, tracking, and lineage modules are not invoked.'];
        end
    ...
    if strcmp(nodeType, 'processor')
        switch pkgName
            ...
            case 'objectmetrics'
                ...
                contract.selectors.channelsParam = '';
                contract.selectors.channelParam = '';
                contract.selectors.outputNameParam = 'outputName';
                contract.parameters.run = {};
                ...
                contract.binding.scope = 'roi';
                contract.binding.outputScope = 'roi';
                contract.binding.mode = 'resources';
                contract.binding.selectorKeys = {'inputData','family'};
                contract.binding.resolveAt = 'run';
                contract.capabilities.roiDataSeries = true;
                contract.capabilities.outputsDataSeries = true;
                contract.resources.in = [ ...
                    resourceDef('dataSeries','metrics','metrics', ...
                        'inputData','dataSeries','inputData',true,''), ...
                    resourceDef('cellModel','cellular_objects','objects', ...
                        'family','objects','family',true,'') ...
                    ];
                contract.resources.out = resourceDef('dataSeries', ...
            ...
            case 'trackmotherlineageviterbi'
                ...
                contract.resources.out = [ ...
                    resourceDef('channel', 'lineage_mother_mask', 'lineage_mother', 'outputChannelName', 'channels', 'outputChannelName', false, 'roiChannel'), ...
                    resourceDef('channel', 'lineage_bud_mask', 'lineage_bud', 'outputChannelName', 'channels', 'outputChannelName', false, 'roiChannel') ...
                    ];
...
function keys = cellLatentTrackerExecutionStaticKeys()
    try
        spec = cellLatentTracker.executionSpec();
        keys = spec.staticKeys;
    catch
        keys = {'topK','frameIntervalMinutes','device', ...
...
function keys = budMotherLinkerExecutionStaticKeys()
    try
        ...
        keys = {'inputFamily','outputFamilyName','frameEnd','minLifetime', ...
            ...
            'trackingLoadGuard','maxNewTracksPerFrame','globalSolver', ...
            'motherRefractoryFrames','youngMotherFrames','solverBeamSize', ...
            'reviewGlobalReassignments', ...
            'overwriteOutputFamily','debug'};
    end
end

function keys = cellLatentModelExecutionStaticKeys()
    semanticKeys = {'backend','temporalVariant','frameIntervalMinutes', ...
    ...
    try
        ...
        keys = unique([semanticKeys normalizeCellstr(spec.staticKeys)], 'stable');
    catch
        keys = [semanticKeys, {'inputFamily','outputFamilyName','frameEnd', ...
            'minLifetime','maxBirthArea','minParentAge', ...
            'maxParentCentroidDistance','maxParentContourDistance', ...
            'maxCandidates','trackingLoadGuard','maxNewTracksPerFrame', ...
            'globalSolver','motherRefractoryFrames','youngMotherFrames', ...
            'solverBeamSize','reviewGlobalReassignments', ...
...
function keys = cnnExecutionStaticKeys()
    try
        spec = cnn.executionSpec();
        keys = spec.staticKeys;
    catch
        keys = {'executionEnvironment'};
    end
end
