...
%% --- Récupération des canaux visibles ---
...
%% --- Détermination des canaux indexés (non-RGB) ---
indexedChannels = [];
...
for i = 1:numChannels
    ...
    if numel(pix) ~= 3  % non-RGB
        if selectedROI.display.indexed(i)==true
...
function arg=gatherArguments(app)
                ...
                arg = [arg, {'Output', dsM.MovieoutputtypeDropDown}];
