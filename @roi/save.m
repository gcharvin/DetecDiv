...
function writeH5CompressedDataset(h5filename, datasetName, data)
% Crée (ou écrase) un dataset dans h5filename avec compression gzip.
% data est un tableau numérique typiquement [H W T] ou [H W C T_sub].
%
% datasetName : string genre '/Channel0' ou '/results_cellposeSAM_1_cell'
    ...
    % Ouvrir ou créer le fichier HDF5
    ...
    % On supprime le dataset s'il existe déjà (sinon H5D.create va râler)
    ...
    dataSize = size(data);
    rank = numel(dataSize);
    dims = fliplr(dataSize);  % HDF5 attend dims dans l'ordre inversé (slowest first)
                              % mais on peut aussi ne pas flipper si on reste cohérent.
                              % Pour rester simple et éviter les transposes chiants :
                              % -> on NE flippe PAS, et on écrit tel quel.
    dims = dataSize;          % on garde l'ordre MATLAB pour simplicité

    % Dataspace
    space_id = H5S.create_simple(rank, fliplr(dims), []); 
    % NOTE: on file les dims dans l'ordre inverse pour HDF5.
    % Trick classique : HDF5 = C-order, MATLAB = column-major.
    % Ici on fait fliplr(dims) pour être cohérent.
    % Donc on a pris dims = dataSize avant, on donne fliplr(dims) à H5S.

    % Propriétés de création du dataset
    dcpl = H5P.create('H5P_DATASET_CREATE');

    % Définir un chunking raisonnable
    % chunk = taille des blocs compressés
    % Heuristique : on chunk par quelques frames
    chunkDims = dims;
    if rank >= 3
        % si data est [H W T] ou [H W C T]
        % on essaie chunk sur ~1-2 frames au lieu de tout T
        chunkDims(end) = min(chunkDims(end), 4); % pas plus de 4 frames par chunk
    end

    H5P.set_chunk(dcpl, fliplr(chunkDims)); % pareil, HDF5 attend dims inversés

    % Activer deflate (gzip). Niveau 4 = compromis vitesse / ratio.
    H5P.set_deflate(dcpl, 4);

    % Type HDF5 de l'array MATLAB
    switch class(data)
        case 'uint8'
    ...
    % Créer le dataset compressé
    dset_id = H5D.create(file_id, datasetName, h5type, space_id, 'H5P_DEFAULT', dcpl, 'H5P_DEFAULT');
...
function tf = h5datasetExists(h5File, dsetPath)
    tf = false;
    if ~exist(h5File,'file')
        return;
    end
    try
        h5info(h5File, dsetPath);
        tf = true;
    catch
        tf = false;
    end
...
%                     hasDataToSave = any(arrayfun(@(ds) isprop(ds, 'groupid') && ~isempty(ds.groupid), data));
%                 elseif isstruct(data)
%                     hasDataToSave = isfield(data, 'groupid') && ~isempty(data.groupid);
%                 else
%                     hasDataToSave = false;
%                 end
%             else
