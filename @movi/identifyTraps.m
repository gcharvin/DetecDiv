function identifyTraps(obj,varargin)

% if option provided, force the generation of new pattern;
...
if nargin>1
    ...
    for i=1:numel(varargin)
        if strcmp(varargin{i},'nosave') % images are not saved on folder
            ss=1;
        end
        
        if strcmp(varargin{i},'pattern') % pattern has to be redefined
            se=1;
          %  'ok'
        end
        
        if strcmp(varargin{i},'display') % results of trap identification provided
            dis=1;
        end
        
        if strcmp(varargin{i},'maxframe') % latest frame to be processed
            maxframe=varargin{i+1};
        end
    end
end
...
img1=obj.readImage(1,obj.PhaseChannel);
obj.imsize=size(img1);

list=dir([obj.pathname{obj.PhaseChannel} '/*.jpg']);
list=[list dir([obj.pathname{obj.PhaseChannel} '/*.tif'])];

if numel(maxframe)==0
obj.nframes=numel(list);
else
obj.nframes=maxframe;    
end
