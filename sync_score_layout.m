...
code = localConfigureCensoringLayout(code);
end

function code = localConfigureCensoringLayout(code)
% Code-owned controls for explicit, human-reviewed quality censoring.
marker = '            % Create NewAnnotationButton';
markerIndex = strfind(code, marker);
if numel(markerIndex) ~= 1
    error('sync_score_layout:CensoringAnchor', ...
        'Could not locate the censoring-control insertion point.');
end
newline = localNewline(code);
blocks = {};
if ~contains(code, 'app.CensorSelectedTrackButton =')
    blocks{end+1} = strjoin({ ... %#ok<AGROW>
    ...
    code = localReplacePropertyLine(code, 'CensorStatusLabel', 'Position', ...
        "            app.CensorStatusLabel.Position = [15 5 230 22];");
end
if ~contains(code, 'app.LineageTreeButton =')
    blocks{end+1} = strjoin({ ... %#ok<AGROW>
...
if ~contains(code, 'app.ReviewFindingsButton =')
    blocks{end+1} = strjoin({ ... %#ok<AGROW>
...
controls = { ...
    ...
    'ReviewFindingsButton', 'uibutton', 'ButtonPushedFcn', ...
        'ReviewFindingsButtonPushed'; ...
    'CensorSelectedTrackButton', 'uibutton', 'ButtonPushedFcn', ...
        'CensorSelectedTrackButtonPushed'; ...
    'LineageTreeButton', 'uibutton', 'ButtonPushedFcn', ...
        'LineageTreeButtonPushed'; ...
...
code = localReplacePropertyLine(code, 'ValidateAnnotationButton', 'Text', ...
    "            app.ValidateAnnotationButton.Text = 'Validate GT';");
if contains(code, 'app.ApproveAnnotationButton.Visible =')
    code = localReplacePropertyLine(code, 'ApproveAnnotationButton', 'Visible', ...
