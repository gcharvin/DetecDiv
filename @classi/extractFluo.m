...
if strcmp(method,'full')
    classiid=obj.strid; %works only for extractFluo as a classi method.
    ...
    for r=rois %to parfor
        ...
        for c=channels
            for t=1:lastFrame
                ...
                obj.roi(r).results.(classiid).fluo.cell.totalf(c,t)=sum(maskedMother(:)-mean(maskedBckg(:)));
                obj.roi(r).results.(classiid).fluo.cell.meanf(c,t)=obj.roi(r).results.(classiid).fluo.cell.totalf(c,t)/sum(maskMother(:));
            end
        end
