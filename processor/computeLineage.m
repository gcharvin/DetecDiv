...
for j=1:2 % loop on training and prediction data
    ...
    if numel(id)~=0
        ...
        if j==1 % collect the probzabilities, but not for groundtruth
        ...
        %    [syncPoint,~]=findSEP(divTimes.duration,1); %find SEP using classical xhi² fit based on div frequency
            ...
            lineage= [NaN, divTimes.lineage, NaN];
            ...
            pixdata=find(arrayfun(@(x) strcmp(x.groupid, ['lineage' nme{j} param.classification_data{end}]),dataout)); % find if object exists already
            %
            if numel(pixdata)
            ...
            plotgroup={'events' 'divisions' 'time' 'count' 'count' 'lineage'};
            ...
            t.Properties.VariableNames={'event', 'divduration' 'totaltime' 'birth' 'death' 'lineage'};

            temp=dataseries(t,{'event', 'divduration' 'totaltime' 'birth' 'death'},...
                'groupid',['lineage' nme{j} param.classification_data{end}],'parentid',roiobj.id,'plot',{true true false false false true},'groups',plotgroup);
...
deathid=find(matches(classes,'dead')); if numel(deathid)==0 deathid=-1; end
clogid=find(matches(classes,'clog'));  if numel(clogid)==0 clogid=-1; end
lbid=find(matches(classes,'large')); if numel(lbid)==0 lbid=-1; end
smid=find(matches(classes,'small')); if numel(smid)==0 smid=-1; end
unbuddedid=find(matches(classes,'unbud')); if numel(unbuddedid)==0 unbuddedid=-1; end
emptyid=find(matches(classes,'empty')); if numel(emptyid)==0 emptyid=-1; end
smbid=find(matches(classes,'smallb'));  if numel(smbid)==0 smbid=-1; end
smtid=find(matches(classes,'smallt')); if numel(smtid)==0 smtid=-1; end
...
firstsmt=find(id==smtid,1,'first');
firstsmb=find(id==smbid,1,'first');
...
frameBirth=min([firstsmt,firstsmb,firstsm,firstlg,firstunb]);
...
if frameFirstEmptiedAfterBirth<param.EmptyThresholdNext%frameFirstEmptiedAfterBirth>param.EmptyThresholdDiscard
    ...
    firstsmt=find(id(frameAfterFirstEmpty:end)==smtid,1,'first');
    firstsmb=find(id(frameAfterFirstEmpty:end)==smbid,1,'first');
    ...
    frameBirth=min([firstunb,firstsm,firstlg,firstsmt,firstsmb]);
end
%
...
frameArrest=NaN;
for arrestid=[unbuddedid,smid,smtid,smbid,lbid]
...
if ~isnan(frameBirth)
    ...
    for j=frameBirth:frameEnd-1
        if (id(j)==lbid && id(j+1)==smid) || (id(j)==unbuddedid && id(j+1)==smid) || (id(j)==lbid && id(j+1)==smtid) || (id(j)==lbid && id(j+1)==smbid)  % bud has emerged
            divFrames=[divFrames j+1];

             if  id(j+1)==smbid
                     lineage=[lineage 1];  % daughter replacement 
             else
                     lineage=[lineage 0]; % mother stays in place
             end
